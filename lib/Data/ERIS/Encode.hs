...
erisEncode ::
  ...
  Int ->
  m ERISCapability
erisEncode db content convergenceSecret blockSize = do
  leafReferenceKeyPairs <- mapM prepareLeafNode leafNodes
  (rootLevel, rootReference, rootKey) <- recurseLevels 1 leafReferenceKeyPairs
  pure $
    ERISCapability
  ...
  where
    ...
    prepareLeafNode :: ByteString -> m (ERISHash, ERISHash)
    prepareLeafNode leafNode = do
      let (block, reference, key) = erisEncryptLeafNode leafNode convergenceSecret
      erisBlockStoragePut db reference block
      pure (reference, key)
    prepareInternalNode :: Word8 -> ByteString -> m (ERISHash, ERISHash)
    prepareInternalNode level node = do
      let (block, reference, key) = erisEncryptInternalNode node level
      erisBlockStoragePut db reference block
      pure (reference, key)
    recurseLevels :: Word8 -> [(ERISHash, ERISHash)] -> m (Word8, ERISHash, ERISHash)
