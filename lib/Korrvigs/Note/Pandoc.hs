module Korrvigs.Note.Pandoc (readNote, readNoteFromText, parsePandoc, parseTopBlocks, extractItem, codeRunnable, parseInlines) where
...
parseInline (Link attr txt (url, _)) = do
  ...
  case T.stripPrefix "mtdt:" url of
    ...
    Nothing ->
      case T.stripPrefix "date:" url of
        Just date -> case parseTimeM True defaultTimeLocale "%Y-%m-%d" (T.unpack date) of
          Just day -> do
            let titleTxt = renderInlines title
            let mtitle = if titleTxt == date then Nothing else Just title
            pure . pure $ A.DateLink mtitle day
          Nothing -> pure []
        Nothing ->
          if T.isInfixOf "://" url
            then do
