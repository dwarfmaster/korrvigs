...
embed lvl syn = do
  let url = syn ^. synUrl
  public <- isPublic
  items :: [(Text, Text, Maybe Id, Maybe UTCTime)] <- rSelect $ orderBy (descNullsFirst $ view _4) $ do
    item <- selectTable syndicatedItemsTable
    where_ $ item ^. sqlSynItSyndicate .== sqlInt4 (syn ^. synEntry . entryId)
    pure (item ^. sqlSynItTitle, item ^. sqlSynItUrl, item ^. sqlSynItInstance, item ^. sqlSynItDate)
  pure
    [whamlet|
    $if lvl == 0
      <p>
