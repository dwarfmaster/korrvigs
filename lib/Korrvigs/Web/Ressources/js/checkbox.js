...
function changeTask(id, prev, nw, stat, upd) {
...
function createCheckboxMenu(elem, st, clickFn) {
    ...
    if(createCheckboxMenu.menu) {
      createCheckboxMenu.menu.remove();
    }
    createCheckboxMenu.menu = menu;
    const unregisterMenu = function () {
      createCheckboxMenu.menu = null;
      menu.remove();
    }
    menu.onmouseleave = () => unregisterMenu();
    document.body.appendChild(menu)
    document.body.addEventListener("click", (ev) => cleanCheckboxMenu(ev, menu, unregisterMenu), false)
    return menu
}
...
function setupCheckbox(postUrl, todoUrl, importantUrl, ongoingUrl, blockedUrl, doneUrl, dontUrl, id) {
  ...
  checkbox.addEventListener('contextmenu', function(ev) {
    ...
    let menu = createCheckboxMenu(checkbox, st, clickFn)
    menu.style = `top:${ev.clientY-10}px;left:${ev.clientX-40}px`
    return false
  })
}

function cleanCheckboxMenu(ev, menu, cleaner) {
  document.body.removeEventListener("click", (ev) => cleanCheckboxMenu(ev, menu, cleaner))
  cleaner()
}
...
function setupTask(postUrl, id) {
  ...
  span.addEventListener('contextmenu', function(ev) {
    ...
    let clickFn = function(nst) {
      ...
      changeTask(id, st, nst, nwName, true)
    }

    let menu = createCheckboxMenu(span, st, clickFn)
    menu.style = `top:${ev.clientY-10}px;left:${ev.clientX-40}px`
    return false
  })
}
...
function setupTopTask(postUrl, id, entryId) {
  ...
  span.addEventListener('contextmenu', function(ev) {
    ...
    let clickFn = function(nst) {
      ...
      changeTask(id, st, nst, nwName, false)
    }
